...
func action(chain: ModLoaderHookChain) -> void:
    var current_attack: CogAttack = chain.reference_object as CogAttack
    if current_attack == null:
        print("Hook reference object is not a CogAttack!")
        return
        
    var user = current_attack.user
    var manager = current_attack.manager
    var targets = current_attack.targets
...
# Determine if attack will hit
    var hit: bool = manager.roll_for_accuracy(current_attack)
...
# Wait for an additional half second to avoid race conditions
    ...
    copy.stats.hp = user.stats.hp # just to be sure
